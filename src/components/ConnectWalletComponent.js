...
export function ConnectWalletComponent() {
  ...
  const walletButtons = (<div><Button onClick={()=>{
    activate(WalletConnectConfig).then(r =>  console.log(r)).catch(e=>console.log(e))}
  }>WalletConnect</Button><br/><Button onClick={()=>{
    activate(Injected).then(r =>  console.log(r)).catch(e=>console.log(e))}
  }>Metamask</Button></div>)
  const connectedChip = <Chip variant="outlined" color="success" icon={<FaceIcon />} label={"Connected @"+account?.substring(0,6)+"..."}/>;
